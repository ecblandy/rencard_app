...
export class PersonalData {
  ...
  toggleTerms(event: Event) {
    ...
    this.signupPersonalModel.update((value) => ({
      ...value,
  ...
  /**
   * Verifica se o carrinho temporário ainda existe.
   */
  loadTempCart() {
    ...
    this.paymentService.getTemporaryCartById(temporaryCart.id).subscribe({
      next: (response) => {
        this.temporaryCart.set(response);
      },
  ...
  onSubmit(event: Event) {
    ...
    submit(this.signupPersonalForm, async () => {
      ...
      try {
        /*
         * O e-mail é necessário porque o usuário ainda está
         * no fluxo de confirmação de cadastro.
         *
         * Não armazenamos senha ou qualquer token aqui.
         */
        localStorage.setItem('pendingEmail', personalData.email);

        /*
         * Define explicitamente que a confirmação é de cadastro.
         */
        sessionStorage.setItem('emailConfirmationType', 'registration');
